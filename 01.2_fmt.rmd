...
```{r echo = FALSE}
table <- cont %>% filter(Treatment == 'Control' |  Treatment == 'Antibiotic FMT')
```

```{r echo=F}
fit=lmerTest::lmer(WBC_zscore ~ ns(age_days, df=5)*Treatment+(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(PMN_zscore ~ ns(age_days, df=5)*Treatment +(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(Lymph_zscore ~ ns(age_days, df=5)*Treatment +(1|animal_number),data=table )
```
...
```{r echo=F}
fit=lmerTest::lmer(Mono_zscore ~ ns(age_days, df=5)*Treatment +(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(Eos_zscore ~ ns(age_days, df=5)*Treatment+(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(NLR_zscore ~ ns(age_days, df=5)*Treatment+(1|animal_number),data=table)
```
