...
```{r echo = FALSE}
table <- cont %>% filter(Treatment == 'Control H1N1' |  Treatment == 'Antibiotic H1N1')
```

```{r echo=F}
fit=lmerTest::lmer(WBC_zscore ~ ns(infection_day, df=5)*Treatment+(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(PMN_zscore ~ ns(infection_day, df=5)*Treatment +(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(Lymph_zscore ~ ns(infection_day, df=5)*Treatment +(1|animal_number),data=table )
```
...
```{r echo=F}
fit=lmerTest::lmer(Mono_zscore ~ ns(infection_day, df=5)*Treatment +(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(Eos_zscore ~ ns(infection_day, df=5)*Treatment+(1|animal_number),data=table)
```
...
```{r echo=F}
fit=lmerTest::lmer(NLR_zscore ~ ns(infection_day, df=5)*Treatment+(1|animal_number),data=table)
```
