...
```{r echo=F}
p=plot_model(fit, type = "eff", title="", terms = c('age_days', 'Treatment'), colors = c("darkgreen", "darkblue"), show.data=TRUE) +
  theme_classic() +
...
```{r echo=F}
# Export csv
write_csv(as.data.frame(anova(fit)) %>% mutate(across(where(is.numeric), round, 3)), path = 'tables/fmt/anova_table.csv')
